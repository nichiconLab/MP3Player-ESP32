...
AudioOutputPWM::AudioOutputPWM(int port, int output_mode, int dma_buf_count, int use_apll)
{
  this->portNo = port;
  this->i2sOn = false;
  ...
  ledcSetup(LEDC_CHANNEL_0, 20000, 10);
  ledcAttachPin(Rout, LEDC_CHANNEL_0);
  ledcWrite(LEDC_CHANNEL_0, 512);
  ledcSetup(LEDC_CHANNEL_1, 20000, 10);
  ledcAttachPin(Rout, LEDC_CHANNEL_1);
  ledcWrite(LEDC_CHANNEL_1, 512);
}
...
bool AudioOutputPWM::SetPinout(int bclk, int wclk, int dout)
{
  bclkPin = bclk;
  wclkPin = wclk;
  doutPin = dout;
...
bool AudioOutputPWM::SetLsbJustified(bool lsbJustified)
{
  this->lsb_justified = lsbJustified;
  return true;
}

bool AudioOutputPWM::begin(bool txDAC)
{
  #ifdef ESP32
    if (!i2sOn)
    {
...
//    return i2s_write_bytes((i2s_port_t)portNo, (const char *)&s32, sizeof(uint32_t), 0);
    ...
    uint16_t valR = Amplify(ms[RIGHTCHANNEL]) + 0x8000;
    uint16_t valL = Amplify(ms[LEFTCHANNEL]) + 0x8000;
    ledcWrite(LEDC_CHANNEL_0, valR >> 6);
    ledcWrite(LEDC_CHANNEL_1, valL >> 6);
